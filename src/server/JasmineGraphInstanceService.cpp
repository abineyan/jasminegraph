...
static void semantic_beam_search(int connFd, InstanceHandler& instanceHandler,
    ...
    // --- Read numHops ---
    content_length = 0;
    instance_logger.info("Waiting for numHops length");

    return_status = recv(connFd, &content_length, sizeof(int), 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading numHops length");
        *loop_exit_p = true;
        return;
    }
    ...
    std::string numHopsStr(content_length, 0);

    return_status = recv(connFd, &numHopsStr[0], content_length, 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading numHops");
        *loop_exit_p = true;
        return;
    }

    int numHops = stoi(numHopsStr);

    instance_logger.info("Received numHops: " + std::to_string(numHops));

    // --- Read beamWidth ---
    content_length = 0;
    instance_logger.info("Waiting for beamWidth length");

    return_status = recv(connFd, &content_length, sizeof(int), 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading beamWidth length");
        *loop_exit_p = true;
        return;
    }
    ...
    std::string beamWidthStr(content_length, 0);

    return_status = recv(connFd, &beamWidthStr[0], content_length, 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading beamWidth");
        *loop_exit_p = true;
        return;
    }

    int beamWidth = stoi(beamWidthStr);

    instance_logger.info("Received beamWidth: " + std::to_string(beamWidth));

    // --- Read initialSeed ---
    content_length = 0;
    instance_logger.info("Waiting for initialSeed length");

    return_status = recv(connFd, &content_length, sizeof(int), 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading initialSeed length");
        *loop_exit_p = true;
        return;
    }
    ...
    std::string initialSeedStr(content_length, 0);

    return_status = recv(connFd, &initialSeedStr[0], content_length, 0);
    if (return_status <= 0) {
        instance_logger.error("Error reading initialSeed");
        *loop_exit_p = true;
        return;
    }

    int initialSeed = stoi(initialSeedStr);

    instance_logger.info("Received initialSeed: " + std::to_string(initialSeed));
...
static void graphrag_command(int connFd, InstanceHandler& instanceHandler,
    ...
    while (std::getline(ss, workerInfo, ',')) {
        ...
        workers.emplace_back(std::move(host), std::stoi(portStr), std::stoi(dataPortStr));
    }
